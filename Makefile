...
clean:
	...
	-rm -f ietf-list-pagination-snapshot@202?-??-??.yang
...
$(next).xml: $(draft).xml
	...
	sed -e"s/YYYY-MM-DD/$(shell date +%Y-%m-%d)/" ietf-list-pagination-snapshot.yang > ietf-list-pagination-snapshot\@$(shell date +%Y-%m-%d).yang
